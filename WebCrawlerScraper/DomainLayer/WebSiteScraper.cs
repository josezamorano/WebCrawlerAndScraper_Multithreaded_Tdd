...
namespace WebCrawlerScraper.DomainLayer
{
    ...
    {
        private static SemaphoreSlim semaphore11 = new SemaphoreSlim(1,1);
            ...
            Thread mainWorkerScraperThread = new Thread(() => 
            {
                ...
                while(_pagesScraped < _totalPagesToScrape)
                {
                    ...
                    Thread fileScraperThread = new Thread(() =>
                    {
                        ...
                        string fileNameScraped = _fileActivityManager.CreateFullPathFileName(fileName, folderNameForScrapedFiles, Utils.Enumerations.FileExtension.csv);
                        bool csvFileCrated = _fileActivityManager.CreateCsvFile(fileNameScraped);
                        ...
                        if (csvFileCrated && countRecordsSaved == allHtmlNodes.Count)
                        {
                ...
                string report = $"{_pagesScraped} Scraped files were created in the selected Folder";
